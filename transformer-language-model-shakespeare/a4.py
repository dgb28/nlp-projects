...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, return_attention_weights=False):
        ...
        """
        # TODO: Implement the Transformer block
        #
        ...
        #
        # Use self.use_pre_norm to determine which architecture to use
        # Use self.attention, self.ffn, self.ln1, self.ln2
        #
        # If return_attention_weights is True, pass it to self.attention and return
        # both the output and the attention weights
        if self.use_pre_norm:
            norm_x = self.ln1(x)
            if return_attention_weights:
                attention_out, attn_weights = self.attention(norm_x, return_attention_weights=True)
            else:
            ...
            if return_attention_weights:
                attention_out, attn_weights = self.attention(x, return_attention_weights=True)
            else:
